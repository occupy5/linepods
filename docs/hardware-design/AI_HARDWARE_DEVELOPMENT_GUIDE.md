...
Initialization reads VERSION and checks the profile update flag plus all 80 profile bytes. When needed, it puts the gauge to sleep, writes and verifies the supplied profile for the specified 520 mAh cell, sets the update flag, and restarts the gauge with the required `0x30` to `0x00` sequence. Replacing the cell requires a matching vendor-generated profile and renewed charge/discharge validation.

That restart makes the gauge recompute its first SOC, which takes a few seconds — notably on every deep-sleep wake, because the terminal path parks the gauge with `CW_CONFIG_SLEEP`. Initialization therefore does not block on the conversion: `bsp_battery_soc()` returns `-1` until the reading is usable, and callers poll it (the Linepods app retries every second until the first usable value, then refreshes every 30 s) while keeping the placeholder on screen.

- SOC uses registers `0x04–0x05`. A value is only returned when the SOC/voltage pair is physically plausible, otherwise the call returns `-1`: values above 100, and `0` while the cell voltage is still at or above `BSP_BATTERY_EMPTY_MV` (3300 mV) — the signature of an unfinished post-restart conversion — are treated as not ready rather than as a real `0 %`.
- Voltage uses the 14-bit value at `0x02–0x03`, converted as `raw × 312.5 µV`, and returned in mV. A pair outside `BSP_BATTERY_MV_MIN`..`BSP_BATTERY_MV_MAX` (2500..4500 mV) is rejected as a bad read.
