...
CW2017 在共享 I2C 地址 0x63。初始化读取 VERSION 确认在线，并检查 profile 更新标志以及全部 80 字节内容。需要更新时，驱动让电量计进入睡眠态，写入并读回校验优特利 520mAh 电芯的厂家 profile，置更新标志，再按 `0x30` 到 `0x00` 的规定时序重启。更换电芯时必须取得匹配的厂家 profile，并重新完成充放电验证。

这次重启会让电量计重算首次 SOC，需要几秒——每次深睡唤醒都会遇到，因为终止流程用 `CW_CONFIG_SLEEP` 把电量计置于睡眠态。因此初始化不再阻塞等待换算：读数可信之前 `bsp_battery_soc()` 返回 `-1`，调用方保持占位符并轮询（Linepods 应用在取得首个可信值之前每秒重试，之后按 30 秒周期刷新）。

- SOC：读 0x04–0x05。只有 SOC 与电压组合物理上成立时才返回读数，否则返回 `-1`：大于 100，以及电芯电压仍在 `BSP_BATTERY_EMPTY_MV`（3300 mV）以上却读到 0（重启后换算未完成的特征），都按未就绪处理，而不是当作真实的 `0 %`。
- 电压：读 0x02–0x03 的 14 bit 值，换算为 `raw × 312.5 µV`，API 返回 mV。SOC 与电压超出 `BSP_BATTERY_MV_MIN`..`BSP_BATTERY_MV_MAX`（2500..4500 mV）的组合按坏读处理。
