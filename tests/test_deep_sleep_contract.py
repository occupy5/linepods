...
class AppDeepSleepContractTest(unittest.TestCase):
    """The Linepods app parks the device in deep sleep after an idle timeout, so
    it must reuse the same terminal shutdown path as the demo."""

    @classmethod
    def setUpClass(cls) -> None:
        cls.app = read("main/main.c")

    def test_app_terminal_shutdown_order_precedes_deep_sleep(self) -> None:
        body = function_body(self.app, "enter_deep_sleep")
        calls = [
            "bsp_battery_sleep()",
            "bsp_audio_sleep()",
            "bsp_audio_prepare_deep_sleep()",
        ...
        self.assertLess(body.index("bsp_lvgl_lock(1000)"),
                        body.index("bsp_display_prepare_deep_sleep()"))
        # The terminal path must never return into the torn-down bus state.
        self.assertIn("esp_restart()", body)

    def test_app_arms_gpio_wakeup_with_a_pin_bitmask(self) -> None:
        body = function_body(self.app, "enter_deep_sleep")
        # A pin *number* (GPIO_NUM_0 == 0) arms nothing; the mask must be built
        # from the BSP pin so the wake source is real and board-specific.
        self.assertIn("esp_deep_sleep_enable_gpio_wakeup(1ULL << BSP_BTN_GPIO",
                      body)
        self.assertIn("ESP_GPIO_WAKEUP_GPIO_LOW", body)
        self.assertNotIn("esp_deep_sleep_enable_gpio_wakeup(GPIO_NUM_0", body)
        # Arming can fail, and sleeping anyway would strand the user.
        self.assertRegex(
            body,
            r"if \(error != ESP_OK\) \{[^}]*?staying awake",
            re.DOTALL,
        )

    def test_app_suspends_radio_before_sleeping(self) -> None:
        body = function_body(self.app, "enter_deep_sleep")
        self.assertIn("linepods_network_suspend_radio()", body)
        self.assertLess(body.index("linepods_network_suspend_radio()"),
                        body.index("esp_deep_sleep_start()"))
