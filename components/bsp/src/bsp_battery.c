...
esp_err_t bsp_battery_init(void) {
    ...
    /* 这里不等首次 SOC 换算：快速启动后的瞬态读数不可信，阻断启动只会把
     * 等待推给整机。bsp_battery_soc() 会一直返回 -1 直到读数可信，上层据此
     * 显示占位符并以更短间隔重试。 */
    ESP_LOGI(TAG, "CW2017 已就绪，等待首次 SOC 换算完成");
...
int bsp_battery_soc(void) {
    ...
    /* 用电压交叉校验：深睡唤醒后的首个换算完成前 SOC 会读到 0，
     * 把它当成真实电量会让开机瞬间显示 0%。 */
    if (!bsp_battery_reading_usable(soc, bsp_battery_mv())) return -1;
    return soc;
}
